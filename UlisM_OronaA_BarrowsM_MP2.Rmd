...
model1 <- lm(Win ~ Eliminations + Deaths, data = COD)
summary(model1)
```
In this model, we are exploring the relationship between Win and Eliminations and Damage. If 
eliminations increases by one then the percentage of winning increases by 1.6%. If damage increases by one winning decreases by 0.14%. 
